...
std::vector<u64> PsiReceiver::run(PRNG &prng, Channel &ch, block commonSeed,
                      const u64 &senderSize, const u64 &receiverSize,
                      const u64 &height, const u64 &logHeight, const u64 &width,
                      std::vector<block> &receiverSet,
                      const u64 &hashLengthInBytes, const u64 &h1LengthInBytes,
                      const u64 &bucket1, const u64 &bucket2) {
  ...
  std::cout << "receiver before receive" << std::endl;
  otExtSender.send(otMessages, prng, ch);
  std::cout << "receiver after receive" << std::endl;
  ...
  for (auto low = 0; low < receiverSize; low += bucket2) {
    ...
    for (auto i = 0; i < width; ++i) {
      for (auto j = low; j < up; ++j) {
        hashInputs[j - low][i >> 3] |=
            (u8)((bool)(transHashInputs[i][j >> 3] & (1 << (j & 7))))
            << (i & 7);
      }
    }
    ...
    for (auto j = low; j < up; ++j) {
      ...
      allHashes[*(u64 *)hashOutput].push_back(
  ...
  for (auto low = 0; low < senderSize; low += bucket2) {
    ...
    for (auto idx = 0; idx < up - low; ++idx) {
      ...
      if (found == allHashes.end())
        continue;

      for (auto i = 0; i < found->second.size(); ++i) {
        if (memcmp(&(found->second[i].first),
                   recvBuff + idx * hashLengthInBytes,
                   hashLengthInBytes) == 0) {
          ++psi;
		  ret.push_back(found->second[i].second);
          break;
        }
      }
    }
  ...
  std::cout << "Receiver sent communication: " << sentData / std::pow(2.0, 20)
            << " MB\n";
  std::cout << "Receiver received communication: "
            << recvData / std::pow(2.0, 20) << " MB\n";
  std::cout << "Receiver total communication: " << totalData / std::pow(2.0, 20)
            << " MB\n";
