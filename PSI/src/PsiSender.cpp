...
void PsiSender::run(PRNG &prng, Channel &ch, block commonSeed,
                    const u64 &senderSize, const u64 &receiverSize,
                    const u64 &height, const u64 &logHeight, const u64 &width,
                    std::vector<block> &senderSet, const u64 &hashLengthInBytes,
                    const u64 &h1LengthInBytes, const u64 &bucket1,
                    const u64 &bucket2) {
  ...
  std::cout << "sender before receive" << std::endl;
  otExtReceiver.receive(choices, otMessages, prng, ch);
  std::cout << "sender after receive" << std::endl;
  ...
  for (auto low = 0; low < senderSize; low += bucket2) {
    ...
    for (auto j = low; j < up; ++j) {
      ...
      memcpy(sentBuff + (j - low) * hashLengthInBytes, hashOutput,
             hashLengthInBytes);
    }
